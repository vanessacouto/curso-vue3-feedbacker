...
export default {
  ...
  setup () {
    ...
    async function handleScroll () {
      // se já passou metada da tela, já pode fazer o request
      ...
      // não tem mais itens para trazer
    ...
    // quando seleciona os filtros
    async function changeFeedbacksType (type) {
      try {
        state.isLoadingFeedbacks = true
        state.pagination.offset = 0
        state.pagination.limit = 5 // reseta a paginacao
    ...
    async function fetchFeedbacks () {
      try {
        ...
        state.feedbacks = data.results // esse 'results' é da resposta da API
        state.pagination = data.pagination
        state.isLoading = false
      } catch (error) {
        handleErrors(error)
